...
# We need to create a params object do use the optional argument for requests' query parameters
params = {
    "q": anime_title,
    "limit": 5
}
...
for index, anime in enumerate(anime_info["data"]):
    print(f"Anime [{index}]: {anime["title"]}")


anime_selected = int(input("Please input the number of the title you want: "))
anime_data = anime_info["data"][anime_selected]
anime_score = anime_data["score"]

print(f"{anime_data["title"]}: {anime_score}")